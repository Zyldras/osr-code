...
def generate_launch_description():
    ...
    roboclaw_params = os.path.join(
        get_package_share_directory("osr_bringup"), "config", "roboclaw_params.yaml"
    )
    ...
    node_servo = Node(
        ...
        respawn=True,
        parameters=[
            {"centered_pulse_widths": [147, 165, 160, 152]}
        ],  # pulse width where the corner motors are in their default position, see rover_bringup.md.
        # indices du tableau :
        # 0 = back_right
        # 1 = front_right
        # 2 = front_left
    ...
    node_teleop = Node(
        ...
        parameters=[
            {
                "scale_linear.x": 1.3
            },  # scale to apply to drive speed, in m/s: drive_motor_rpm * 2pi / 60 * wheel radius * slowdown_factor
            {"axis_linear.x": 1},  # ! xbox=1 !
            {
                "axis_angular.yaw": 0
            },  # ! xbox=0 ! which joystick axis to use for driving
            {"axis_angular.pitch": 2},  # ! xbox=2 ! axis to use for in-place rotation
            {
                "scale_angular.yaw": 2.88
            },  # scale to apply to angular speed, in rad/s: scale_linear / min_radius(=0.45m)
            {
                "scale_angular.pitch": 2.88
            },  # scale to apply to angular speed, in rad/s: scale_linear / min_radius(=0.45m)
            {
                "scale_angular_turbo.yaw": 3.88
            },  # scale to apply to angular speed, in rad/s: scale_linear_turbo / min_radius
    ...
    node_joy = Node(
        ...
        parameters=[
            ...
            {
                "device_id": 0
            },  # ! default = 0 ! This might be different on your computer. Run `ls -l /dev/input/event*`. If you have event1, put 1.
        ],
    )
    ...
    # node_joy_extras = Node(
    #     package="osr_control",
    #     executable="joy_extras",
    #     output="screen",
    #     emulate_tty=True,
    #     parameters=[{"duty_button_index": 1}],  # which button toggles duty mode on/off
    # )
    ...
    launch_rf2o = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [
                os.path.join(
